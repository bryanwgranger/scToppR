...
Citations: https://toppgene.cchmc.org/help/publications.jsp"
    ...
    if (type == "degs") {
        ...
        if (!(cluster_col %in% colnames(input_data))) {
            msg <- paste0("Cluster column `", cluster_col, "` not found in data. Please specify.")
            stop(msg)
        }
        # parse fc_filter
        ...
        if (direction_mode == "all") {
            ...
            gene_data_split[["up"]] <- .process_degs(
                degs = input_data_up,
                cluster_col = cluster_col,
                gene_col = gene_col,
                p_val_col = p_val_col,
            ...
            gene_data_split[["down"]] <- .process_degs(
                degs = input_data_down,
                cluster_col = cluster_col,
                gene_col = gene_col,
                p_val_col = p_val_col,
                logFC_col = logFC_col,
    ...
    if (length(missing_clusters) > 0) {
        msg <- paste("No results for clusters:", paste(missing_clusters, collapse = ", "))
        warning(msg)
    }
    return(big_df)
}
...
toppSave <- function(toppData,
    ...
    if (isTRUE(split)) {
        if (!(cluster_col %in% colnames(toppData))) {
            msg <- paste0("Cannot split by cluster column `", cluster_col, "` as it is not found in toppData. Please specify the correct column name.")
            stop(msg)
        }
